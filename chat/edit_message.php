...
// fetch message
$stmt = $conn->prepare("SELECT sender_id, created_at, message_text FROM messages WHERE message_id = ?");
...
$update = $conn->prepare("UPDATE messages SET message_text = ?, edited_at = NOW(), edited_count = edited_count + 1 WHERE message_id = ?");
if ($update->execute([$new_text, $message_id])) {
    $hist = $conn->prepare("INSERT INTO message_edits (message_id, old_text, new_text, edited_by) VALUES (?, ?, ?, ?)");
